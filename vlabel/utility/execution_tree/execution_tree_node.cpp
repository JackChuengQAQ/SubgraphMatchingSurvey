...
void no_partition_hash_join_node::execute_operator(std::vector<flat_relation *> &relations, size_t& call_count) {
    output_count_ = nop_join_->execute(call_count);
...
void leap_frog_trie_join_node::execute_operator(std::vector<flat_relation *> &relations, size_t& call_count) {
    output_count_ = lftj_->execute(call_count);
