...
enum WriteFlag {
    ...
    kWriteDefaultFlags = CEREAL_RAPIDJSON_WRITE_DEFAULT_FLAGS  
};
...
public:
    ...
    bool EndObject(SizeType memberCount = 0) {
        ...
        CEREAL_RAPIDJSON_ASSERT(0 == level_stack_.template Top<Level>()->valueCount % 2); 
...
#undef Z16
    ...
    void Prefix(Type type) {
        ...
        if (CEREAL_RAPIDJSON_LIKELY(level_stack_.GetSize() != 0)) { 
            ...
            CEREAL_RAPIDJSON_ASSERT(!hasRoot_);    
            hasRoot_ = true;
        }
    }
    ...
    bool EndValue(bool ret) {
        if (CEREAL_RAPIDJSON_UNLIKELY(level_stack_.Empty()))   
            Flush();
        return ret;
    }
