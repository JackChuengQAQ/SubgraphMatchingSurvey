...
void 
EvaluateQuery::pruneCandidatesIndexBySymmetryBreaking(ui depth, ui *embedding, ui* order,
...
void 
EvaluateQuery::pruneCandidatesBySymmetryBreaking(ui depth, ui *embedding, ui* order,
...
bool 
EvaluateQuery::checkSingeVertexBySymmetryBreaking(ui depth, ui *embedding, ui* order, VertexID v,
                                       const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>>& ordered_constraints) {
    VertexID u = order[depth];
    ...
    if (it != ordered_constraints.end()) {
        ...
        for (auto& u_p: u_cons_prior) {
            if (embedding[u_p] > v) return false;
        }

        for (auto& u_i: u_cons_inferior) {
            if (embedding[u_i] < v) return false;
        }
    }
    return true;
}
...
bool 
EvaluateQuery::checkSingeVertexBySymmetryBreaking(ui depth, ui *embedding, ui*order, bool* visited_u, VertexID v,
                                        const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>>& full_constraints,
...
void 
EvaluateQuery::AncestorWithSymmetryBreaking(const Graph *query_graph,
                             std::vector<std::bitset<MAXIMUM_QUERY_GRAPH_SIZE>> &ancestors,
                             const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>>& ordered_constraints) {
    ui query_vertices_num = query_graph->getVerticesCount();
