...
uint64_t execution_tree::execute(catalog &catalog, uint64_t output_count_limit, size_t& call_count) {
    uint64_t count = 0;
    for (auto node : tree_nodes_) {
        count = node->execute(catalog, relations_, output_count_limit, call_count);
    }
    return count;
}