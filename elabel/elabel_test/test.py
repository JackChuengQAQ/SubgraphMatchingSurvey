...
def check_correctness(binary_path, data_graph_path, query_folder_path, expected_results):
    ...
    # check the correctness of VEQ engine.
    for query_graph_path in query_graph_path_list:
        execution_args = generate_args(binary_path, '-d', data_graph_path, '-q', query_graph_path, '-filter', 'VEQ',
                                       '-order', 'GQL', '-engine', 'VEQ')
        ...
        if rc == 0:
            ...
            for line in std_output_list:
                if '#Embeddings' in line:
                    ...
                    break
            if embedding_num != expected_embedding_num:
                print 'VEQ engine {0} is wrong. Expected {1}, Output {2}'.format(query_graph_name,
                        expected_embedding_num, embedding_num)
                exit(-1)
        else:
            print 'VEQ engine {0} error.'.format(query_graph_name)
            exit(-1)
    print 'VEQ engine pass the correctness check.'

    # check the correctness of VF3 engine.
    for query_graph_path in query_graph_path_list:
        execution_args = generate_args(binary_path, '-d', data_graph_path, '-q', query_graph_path, '-filter', 'NLF',
                                       '-order', 'VF3', '-engine', 'VF3')
        ...
        if rc == 0:
            ...
            for line in std_output_list:
                if '#Embeddings' in line:
                    ...
                    break
            if embedding_num != expected_embedding_num:
                print 'VF3 engine {0} is wrong. Expected {1}, Output {2}'.format(query_graph_name,
                        expected_embedding_num, embedding_num)
                exit(-1)
        else:
            print 'VF3 engine {0} error.'.format(query_graph_name)
            exit(-1)
    print 'VF3 engine pass the correctness check.'

    # check the correctness of RapidMatch engine.
    for query_graph_path in query_graph_path_list:
        execution_args = generate_args(binary_path, '-d', data_graph_path, '-q', query_graph_path, '-filter', 'RM',
                                       '-order', 'RM', '-engine', 'RM')
        ...
        if rc == 0:
            ...
            for line in std_output_list:
                if '#Embeddings' in line:
                    ...
                    break
            if embedding_num != expected_embedding_num:
                print 'RM engine {0} is wrong. Expected {1}, Output {2}'.format(query_graph_name,
                        expected_embedding_num, embedding_num)
                print(std_output)
                print(std_error)
                exit(-1)
        else:
            print 'RM engine {0} error.'.format(query_graph_name)
            print(std_output)
            print(std_error)
            exit(-1)
    print 'RM engine pass the correctness check.'

    # check the correctness of KSS engine.
    for query_graph_path in query_graph_path_list:
        execution_args = generate_args(binary_path, '-d', data_graph_path, '-q', query_graph_path, '-filter', 'DPiso',
                                       '-order', 'DPiso', '-engine', 'KSS')
        ...
        if rc == 0:
            ...
            for line in std_output_list:
                if '#Embeddings' in line:
                    ...
                    break
            if embedding_num != expected_embedding_num:
                print 'KSS engine {0} is wrong. Expected {1}, Output {2}'.format(query_graph_name,
                        expected_embedding_num, embedding_num)
                print(std_output)
                print(std_error)
                exit(-1)
        else:
            print 'KSS engine {0} error.'.format(query_graph_name)
            print(std_output)
            print(std_error)
            exit(-1)
    print 'KSS engine pass the correctness check.'
